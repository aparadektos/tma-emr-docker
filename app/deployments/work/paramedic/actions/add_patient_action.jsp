...
{
    for(CountryBean curCountryBean : paramedicBacking.getAllCountriesAndNationalitiesList())
    {
...
else
{
    ...
    {
        paramedicBacking.infoMessage=langBacking.getLiteral("SSN or OTHER");
        response.sendRedirect("../addNewPatient.jsp");
    }
}
...
if(shouldProceed)
{
       ...
       (patDocDeclaration!=null && patDocDeclaration.equalsIgnoreCase("on"))) )
    {
        ...
        {
            if(paramedicBacking.ssnExists(patSSN,null)==false)
            {
                //insert new patient to DB table
                if(paramedicBacking.insertNewPatient(patientBean)==true)
                {
                    //if success response OK 
                    paramedicBacking.okMessage=langBacking.getLiteral("new_patient_saved");
                    response.sendRedirect("../patients.jsp");
                }
                else
                {
                    //if failed response ERROR 
                    paramedicBacking.errorMessage=langBacking.getLiteral("add_patient_failed");
                    response.sendRedirect("../addNewPatient.jsp");
                }
            }
            else
