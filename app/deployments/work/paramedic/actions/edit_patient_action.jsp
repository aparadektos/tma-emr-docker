...
if(ssnSubmitted)
{
    if(ssnValid==false)
    {
        paramedicBacking.infoMessage=langBacking.getLiteral("ssn_format");
...
else
{
    ...
    {
        paramedicBacking.infoMessage=langBacking.getLiteral("ssn_or_other");
        response.sendRedirect("../editPatient.jsp?patId="+patId);
    }
}
...
if(shouldProceed)
{
    ...
    {    
        ...
        patBean patientBeanToEdit=paramedicBacking.getPatientById(patId);
...
//        patientBeanToEdit.address=patAddress;
        ...
        if(patSSN!=null && patSSN.length()>0)
        {
            if(paramedicBacking.ssnExists(patSSN,patientBeanToEdit.id)==false)
            {
                //update patient to DB table
                if(paramedicBacking.updatePatient(patientBeanToEdit)==true)
                {
                    //if success response OK 
                    paramedicBacking.okMessage=langBacking.getLiteral("edit_patient_ok");
                    paramedicBacking.patientSearchResults=new ArrayList<patBean>(0);
                    paramedicBacking.patientSearchResults.add(patientBeanToEdit);
                    response.sendRedirect("../patients.jsp");
                }
                else
                {
                    //if failed response ERROR 
                    paramedicBacking.errorMessage=langBacking.getLiteral("edit_patient_failed");
                    response.sendRedirect("../editPatient.jsp?patId="+patId);
                }
            }
            else
            {
                paramedicBacking.infoMessage=langBacking.getLiteral("ssn_exists");
                response.sendRedirect("../editPatient.jsp?patId="+patId);
            }
        }
        else
    ...
    {
        paramedicBacking.infoMessage=langBacking.getLiteral("edit_patient_required_fields");
        response.sendRedirect("../editPatient.jsp?patId="+patId);
    }
}
